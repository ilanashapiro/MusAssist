...
updateBeat noteDuration timePerMeasure state = do
  let beatCt = fst state
      measureCt = snd state
  beatCount <- Data.IORef.readIORef beatCt
  let updatedBeat = beatCount + noteDuration 
  if updatedBeat == timePerMeasure 
    then do
...
transNote :: Counter -> MusAST.Note -> IO [CodeLine]
------------------------------------------------
-- Rests
------------------------------------------------
transNote ct t0 (MusAST.REST duration) = do
...
-- General notes
------------------------------------------------
transNote ct t0 (MusAST.Note noteName accidental octave duration) = do
  let noteDuration = convertDurationToFloat duration
