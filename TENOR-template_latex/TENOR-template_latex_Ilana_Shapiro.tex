...
If we instead have an arpeggio, we first generate the notes in the underlying chord using the above logic. If the arpeggio is specified as a triad, we then double the root an octave up in order to have a complete arpeggio. Finally, we sort the tones in the arpeggio so that they strictly ascend or descend depending on the specified arpeggio direction, and return them as a list of individual tones, rather than a simultaneous cluster as in a chord.
