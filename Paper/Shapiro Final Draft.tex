...
These commands,  respectively, translate to the following:

\begin{figure}[h!]
\centering
\includegraphics[width=0.38\textwidth]{cmintriad}
\caption{C minor triad, first inversion, 6th octave}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{f7}
\caption{F\# half diminished seventh, third inversion, 4th octave}
\end{figure}
...
\noindent where \param{NOTENAME}, \param{OCTAVE}, and \param{DURATION} (but not \verb.<ACCIDENTAL>.) are taken from the same options as in \Cref{sec:notes}.
...
\noindent This is because together, \verb.<NOTENAME><ACCIDENTAL><OCTAVE> <QUALITY>. determine the key (and the start octave) of the harmonic sequence, and a key can only be major or minor. 

Like chord templates, here \param{ACCIDENTAL} is taken from one of the following restricted options:
\begin{verbatim}
#, b
\end{verbatim}

\noindent This is because a key in MusAssist cannot be described with a double sharp or flat, as such key signatures may introduce triple sharps or flats, which recall are not supported. \param{ACCIDENTAL}  continues remain optional, with natural implied when it is left out.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{ascfifths}
  \caption{Ascending Fifths Sequence}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{descfifths}
  \caption{Descending Fifths Sequence}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{asc56}
  \caption{Ascending 5-6 Sequence}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{desc56}
  \caption{Descending 5-6 Sequence}
\end{figure}


The figures were created with the following MusAssist code, respectively: \\\verb.(AscFifths C4 maj quarter length:15)., \verb.(DescFifths C4 maj quarter length:15)., \\\verb.(Asc56 C4 maj quarter length:15)., \verb.(Desc56 C6 maj quarter length:15).
\begin{verbatim}
 

\end{verbatim}

In most settings, the ascending fifths sequence would terminate after five chords (i.e. before it reaches a root position diminished  triad). However, MusAssist does not stop at this chord based on the principle that users should be able to generate sequences of any length. Terminating this sequence after five chords is up to the user.
...
Parsec
	Token and Combinators
	String literal prefix issue
	Abstract syntax
...
The abstract syntax of the parse result contains information about various descriptions of musical objects, such as harmonic sequences and chord templates. 
