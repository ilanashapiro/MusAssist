...
This chapter details the concrete syntax of MusAssist. In the following sections, parameters take the following form: \verb!<PARAMETER>!.
%----------------------------------------------------------------------------
\section{Rests}
\label{sec:rests}
...
\noindent where \verb!<DURATION>! is taken from one of the following options
...
\noindent where \verb!<NOTENAME>! is taken from one of the following options
\begin{verbatim}
C, D, E, F, G, A, B
\end{verbatim}

\noindent and \verb!<ACCIDENTAL>! is taken from one of the following options
\begin{verbatim}
#, ##, b, bb
\end{verbatim}

\verb!<ACCIDENTAL>! is an optional parameter. If the user leaves it out, the note is considered natural. Notice that double sharps are represented with \verb!##! rather than the conventional \musDoubleSharp \; symbol, as this is not an easily typed out character on a keyboard.

\verb!<OCTAVE>! is taken from one of the following options.
\begin{verbatim}
1,2,3,4,5,6,7,8
\end{verbatim}
\noindent as these are the octaves of a standard piano.

\verb!<DURATION>! is taken from the same options as in \Cref{sec:rests}.
...
\noindent where \verb!<NOTENAME>!, \verb!<ACCIDENTAL>!, \verb!<OCTAVE>!, and \verb!<DURATION>! are taken from the same options as in \Cref{sec:notes}, and \verb!<ACCIDENTAL>! is similarly optional, with natural implied when it is left out.

\verb![<NOTENAME><ACCIDENTAL><OCTAVE>]! indicates an arbitrarily long list supplied by the user, where each of the values are of the form \verb!<NOTENAME><ACCIDENTAL><OCTAVE>!
...
Examples of MusAssist chord templates are \verb!(C6 min triad inv:first quarter)! and  \\\verb!(F#4 halfdim seventh inv:third eighth)!
