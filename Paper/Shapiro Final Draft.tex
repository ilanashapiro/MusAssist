...
Harmonic sequences were the most complex template to expand. In order to generate a harmonic sequence, two pieces of information need to be determined: (1) the interval of the next chord relative to the previous, and (2) the octave number of each chord.
...
In each figure, the top row is the chord progression, the middle row is the interval of the chord from the tonic, and the bottom row is the interval of each chord from the previous (modulo 7). A clear pattern emerges for each sequence. Thus, given the index of a chord, we can definitely determine its interval relationship to the previous chord in the sequence.
...
Unfortunately, however, the functions from chord index to special octave cases is not 1-1. 
