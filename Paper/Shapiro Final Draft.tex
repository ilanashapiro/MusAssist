...
\end{figure}
 ...
 \Cref{fig:asc_fifths_octave_grid} was then transposed and reorganized as follows to relate indices to key names. Note that \verb.succ2. is shorthand for \verb!succ . succ!
...
Static MusicXML header code is generated for all MusAssist files that handles encoding, print and midi settings, and layout defaults. The header code can be found \href{https://github.com/ilanashapiro/MusAssist/blob/main/app/Compile.hs}{here}.  

However, a critical part of the MusicXML header is left off the static header code: the program attributes. 
...
Finally, notice the \verb.<divisions>4</divisions>. element at the top. The \verb.<divisions>. tag is critical to the representation of note durations throughout the MusicXML program. The \verb.<divisions>. value 
...
Notice that a rest is a special kind of \verb.<note>. element, with the \verb.<rest/>. tag instead of pitch information. The \verb.<voice>. value is set to 1 for all MusAssist elements, as the voice type is a more advanced notation element that MusAssist does not currently support. Multiple voicing would mean the ability to have simultaneous lines occurring in a single measure, which is certainly a features MusAssist could strive to support in the future.
