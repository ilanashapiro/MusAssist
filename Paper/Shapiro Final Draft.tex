...
Finally, a $ language$ $workbench$ is a customized IDE for building and defining DSLs \citeparen{fowler_parsons_2011}. In this paper, language workbenches are not discussed; the emphasis is purely on external and internal DSLs.

DSLs for music have not been studied as extensively as other application domains, but are a fascinating area of inquiry that explores the expressive power of languages and pushes the boundaries of computational creativity. The era of music DSLs began in 2008  with Ge Wang's invention of the ChucK audio processing language. ChucK is actually a GPL, though one broadly tailored towards music. With ChucK, Wang developed a language that is "expressive and easy to write and read with respect to time and parallelism," thus providing users with a "platform for precise audio synthesis/analysis and rapid experimentation in computer music." \citeparen{wang_2008}

 Music itself has a highly structured framework, and a musical score itself can be thought of as having many of the formal features of a PL. With the increased flexibility afforded to  a DSL via its limited expressiveness, it can be much  more effectively tailored to the application (i.e. music) than an GPL could be. Depending on the goals of the programmer, a DSL can be used to target a particular aspect of music, whether that be notation, algorithmic composition, signal processing, or live coding with music performance.  For instance, as will be discussed in \Cref{chap:background}, \Cref{subsec:comp_counterpt_marks},  Computational Counterpoint Marks is a music DSL designed by Juan Carlos Martinez to enhance Western music notation. The ``counterpoints marks" of Martinez's DSL are seen below the  ``source code" of the music itself -- here, Steve Reich's  \textit{Piano Phase} -- in the following example:

\begin{figure}[h!]

\centering
  \includegraphics[width=0.5\textwidth]{piano_phase}
\caption{Computational Counterpoint Marks and Steve Reich's \textit{Piano Phase}}
\label{piano_phase}
\end{figure}
\newpage

Figure \ref{piano_phase} demonstrates how Computational Counterpoint Marks help to reinterpret  Western music notation  computationally, thus creating an augmented version of the performance by delineating a complicated computer-music interaction that is  comprehensible by both human  and  machine \citeparen{martinez_2021}. This task is  something that a GPL would be hard-pressed to achieve  in an elegant, efficient way. The limited expressiveness of Computational Counterpoint Marks thus allows it to be highly specialized. 
...
Musical markup languages such as Michael Good's MusicXML and LilyPond fall under this category. 
...
\subsection{LCSynth}
Recall from Section 2.4 the DSL LSynth, created by Nishimo et al, that was inspired by Wang's strongly-timed DSL ChucK. 
